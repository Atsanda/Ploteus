...
QVector<float> linear_aproximation(QVector<float> list_of_x, QVector<float> list_of_y){
    ...
    QVector<float> coefficient_of_linear_aproximation;

    for (QVector<float>::iterator iter = list_of_x.begin(); iter != list_of_x.end(); iter++)
	{
		sum_x = sum_x + *iter;
	}

    for (QVector<float>::iterator iter = list_of_y.begin(); iter != list_of_y.end(); iter++)
	{
		sum_y = sum_y + *iter;
	}
